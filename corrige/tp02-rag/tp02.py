...
def charger_memoire(memoire_brute: list[dict]) -> list[dict]:
    ...
    return [segment for segment in memoire_brute if segment["statut"] == "valide"]
    # >>>CODE 1
...
def glossaire_pertinent(segment_src: str, index: GlossaireVectoriel,
    ...
    retenus = []
    for terme, vecteur in zip(index.glossaire, index.vecteurs):
        score = max((cosinus(v, vecteur) for v in vecteurs), default=0.0)
        if score >= seuil:
            retenus.append({**terme, "score": score})
...
def construire(segment_src: str, memoire: list[dict], index: GlossaireVectoriel,
    ...
    return construire_messages(segment_src, voisins=voisins, glossaire=termes,
